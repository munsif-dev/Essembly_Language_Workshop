...
;cmp al,3                ; al - 3   it like substruction, and it compares but not return Value
                        ; it is like a flag setter


;jmp label       ; change or make next Instruction to be executed 
                ; make the cpu executed the next instruction at this address in the label
...
section .data  
    ...
    msg3 db 0xa, 0xb, "THere is an Error in the program" ; Result message
    msg3_len equ $ - msg3                 ; Length of the result message
...
_start:
    ...
    cmp al, '9'                     ; jmp to ENDPROG if al>9
    jg ERRORMSG
...
ERRORMSG:
    mov eax, 4                            ; syscall number for sys_write (write)
    mov ebx, 1                            ; file descriptor 1 is stdout
    mov ecx, msg3                         ; pointer to the result message
    mov edx, msg3_len                     ; length of the result message
    int 0x80     
    jmp ENDPROG                             ; Here the actual Unconditional jump happen